...
@ApplyOptions<CommandOptions>({
    name: "resume",
    description: "resume giveaway in current channel or different channel",
    requiredClientPermissions: ["SEND_MESSAGES", "EMBED_LINKS"],
    detailedDescription: "usage: g!resume 888728606640078879",
    ...
    public async run(message: Message, args: Args) {
        ...
        if (!giveawayId) {
            return message.reply({
                embeds: [
                    new MessageEmbed()
                        .setDescription("Please input message id/reply message you want to resume!")
                ]
            })
        }
        ...
        //@ts-ignore
        this.container.client.giveawayManager.unpause(giveawayValue.messageId!);
    }
}